...
export function PageContextHeader(props: { title: string }) {
  ...
  return <div>
    ...
    {site &&  
    <div className="flex"><div>site:</div>  <Link href={`/koksmat/tenants/${tenant}/site`}>{site}</Link>  </div>}
{site &&  
      <div className="flex"><div>kitchen:</div>  <Link href={`/koksmat/tenants/${tenant}/site/${site}/kitchen`}>{kitchen? kitchen:"<select>"}</Link> </div>
}
{kitchen &&
      <div className="flex"><div>station:</div>  <Link href={`/koksmat/tenants/${tenant}/site/${site}/kitchen/${station}`}>{station? station:"<select>"}</Link> </div>
    }
