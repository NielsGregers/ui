	...
	export function ItemForm(props: { item?: ItemType }) {
		...
		/><FormField
			control={form.control}
			name="LocationonFloor"
			render={({ field }) => (
				<FormItem>
					<FormLabel>Location on Floor</FormLabel>
					<FormControl>
		...
		/><FormField
			...
			render={({ field }) => (
				<FormItem>
					<FormLabel>Floor Map Zoom</FormLabel>
					<FormControl>
						<Input placeholder="" {...field} value={field.value??"" }/>
					</FormControl>
					<FormDescription>
		...
		/><FormField
			...
			render={({ field }) => (
				<FormItem>
					...
					<FormDescription>
						If this is a Teams Meeting room, this number  is used to identify the device towards Office 365
					</FormDescription>
					<FormMessage />
				</FormItem>
			)}
